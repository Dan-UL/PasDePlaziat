...
La figure ci-dessus (figure 6) montre que le `nodesize` qui minimise l'RMSE se trouve autour de 80. Nous allons donc tester toutes les valeurs de 60 à 100 pour trouver la valeur optimale de l'hyperparamètre.
...
Dans la figure ci-dessus (figure 7), nous pouvons voir que la valeur de `nodesize` optimale est 97.
...
Nous voyons, dans la figure 11, qu'une profondeur de 15 et qu'environ 4000 arbres minimisent la REQM. Une profondeur de 15 semble suffisante pour capter les itérations entre les variables, sans trop faire exploser le temps de calcul. Il est difficile de voir le nombre idéal.
...
![Importances des variables pour le bagging](importance_fa.jpg)

\vspace{8pt}

Selon la figure 12, la variable la plus importantes pour le modèle *bagging* est `totalCoverage`.Pour mieux comprendre son effet marginal sur la prévision, nous visualisons le graphique de dépendance partielle.
