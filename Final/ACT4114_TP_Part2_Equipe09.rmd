...
```{r, cache=TRUE, echo = F, warning=F, fig.cap= c("MSE selon le nombre d'arbre"), fig.dim= 0.5}
...
Pour éviter un surajustement dû à des arbres trop profonds, on devra ajuster la valeur de `nodesize`, mais il est impossible de le faire directement avec le package `caret`. Puisque le modèle est entraîné sur `r nrow(dat.train)`` données, les valeurs de 100 et moins seront testées par bonds de 5.
