...
ggplot(data, aes(x = condominiumIndicator, y = log(totalAmount), col = condominiumIndicator))+
  geom_boxplot()+
  theme_minimal()+
  labs( x = "Est un condominium",y = "Montant total (échelle log)")+
  theme(legend.position = "none")
...
Variable indicatrice, 1 si l’habitation est un condominium, 0 sinon.
...
df <- data.frame(rbind(
  ...
  "25% - 35%" = c(min(data$totalAmount[data$communityRatingSystemDiscount == 3]),
   ...
   "Plus de 40%" = c(min(data$totalAmount[data$communityRatingSystemDiscount == 4]),
...
Cette variable représente le pourcentage de rabais accordé lors de la tarrification. 

On remarque qu'il y a un trop grand nombre de catégories. Pour cette raison, elles sont réunies en quatres catégories de crédit:
...
Cette variable temporelle, au jours près, indique la date où s’est produit l’infiltration d’eau dans le bâtiment.

```{r}
ggplot(data, aes(x = dateOfLoss, y = totalAmount))+
  geom_point(alpha = 0.4)+
  theme_minimal()+
  labs(x = "La date de l'infiltration d'eau", y = "Montant total" )
```
...
ggplot(data, aes(x = elevatedBuildingIndicator, y = log(totalAmount), col = elevatedBuildingIndicator))+
  geom_boxplot()+
  theme_minimal()+
  labs( x = "Le bâtiment est élevé",y = "Montant total (échelle log)")+
  theme(legend.position = "none")
```
...
Variable indicatrice, 1 si le bâtiment est élevé, c’est-à-dire, au dessusdu nieveau du sol, 0 sinon.
Un bâtiment est considéré élevé s'il le plancher le plus bas est au dessus du niveau du sol.

## Variables *latitude* et *longitudes*
...
ggplot(data = data, aes(x = longitude, y = latitude, col= log(totalAmount))) +
    mapCalifornia + geom_point(alpha = .6) + scale_color_gradientn(colors = c("#FC4E07", "#E7B800","darkolivegreen2", "#00AFBB"))+
  theme_minimal()+
  labs(x= "Longitude", y="Latitude", title = "Réclamation en forction de la localisation", col ="Montant total (échelle log)")
...
Variable catégoritielle indiquant où se trouve le contenu du bâtiment assuré qui a été endomagé.

On remarque pour la variable locationOfContents, qu'il y a plusieurs catégories ne comportant qu'un faible nombre d'observations. Certaines seront donc combinées pour former les nouvelles catégories suivantes:
...
ggplot(data.x, aes(x = lowestFloorElevation, y = totalAmount))+
  geom_point(alpha = 0.4)+
  theme_minimal()+
  labs(x = "La hauteur du plus bas étage (Pieds)", y = "Montant total", title = "Montant des réclamations en fonction de l'élévation de plus bas étage" )
...
Cette variable représente la hauteur du plus bas étage de l'habitation, en pieds.
...
Cette variabe comportait 6 catégories d'habitation, nous avons conservé les trois premiers niveaux, et nous avons combinés les trois dernières selon leur nombre d'étages. Nous avons ainsi obtenu les niveaux suivants :
...
df <- data.frame(rbind(
  "Couverture d'assurance" = c(min(data$totalCoverage),
                  max(data$totalCoverage))
  ))
...
Dans le jeu de données, les montants de couverture (totalBuildingInsuranceCoverage & totalContentsInsuranceCoverage) pour le bâtiment et pour les biens personnels étaient séparés, nous avons jugés plus logique de combiner les deux variables en une (totalCoverage), puisque nous avions déjà réunis les montants des réclamations.
...
ggplot(data, aes(x = primaryResidence, y = log(totalAmount), col = primaryResidence))+
  geom_boxplot()+
  theme_minimal()+
  labs( x = "Le bâtiment est une résidence primaire",y = "Montant total (échelle log)")+
  theme(legend.position = "none")
```
...
Variable indicatrice indiquant si le bâtiment assuré est la résidence primaire, 1 si oui, 0 sinon.
